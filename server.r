...
server <- function(input, output) {
  output$SEAdisaggsview <- renderUI({
    ...
    tabBox(
      title = "Intersectionality Output", width = 12, id = "SEAgentabbox",
      tabPanel(
        title = "Outcomes", value = "outcomes",
        SEAgengraphmodUI("SEAgengraph")
      ),
      tabPanel(
        title = "Equity Number", value = "eqnumbers",
        SEAgengraphmodUI("SEAgennumbergraph")
      ),
      tabPanel(
        title = "PPG-1", value = "ppg",
  ...
  SEAgraphout <- reactive({
    if (input$SEAtabbox == "outcomes") {
      ...
      SEAfiltered() %>%
        mutate(
          yaxis = primary_full_equity_number,
  ...
  SEAgengraphout <- reactive({
    if (input$SEAgentabbox == "outcomes") {
      ...
      SEAgenfiltered() %>%
        mutate(
          yaxis = gender_intersectional_full_equity_number,
  ...
  SEAtabledata <- reactive({
    SEAgraphout() %>%
      select(
        academic_year, metric_description, primary_disagg_subgroup,
  ...
  SEAgentabledata <- reactive({
    SEAgengraphout() %>%
      select(
        academic_year, metric_description, primary_disagg_subgroup, gender_disagg_subgroup,
